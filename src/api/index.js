...
const API = {
    ...
    signin: (phone, code) => Promise.resolve({ data: { accessToken: 123456 }, user: { name: "nata", phone: "+573007988200" } }),
    signup: (name, phone, code) => Promise.resolve({ data: { accessToken: 123456 }, user: { name: "nata", phone: "+573007988200" } }),
    agendas: (fecha) => fetch(`${process.env.REACT_APP_API}/agendas/${fecha}?access_token=${geAccessToken()}`).then(body => body.json()),
    agendasWeek: (fechaIni, fechaFin) => fetch(`${process.env.REACT_APP_API}/agendas/${fechaIni}/${fechaFin}?access_token=${geAccessToken()}`).then(body => body.json()),
    ...
    balanceWeek: (fechaIni, fechaFin) => fetch(`${process.env.REACT_APP_API}/agendas/balance/${fechaIni}/${fechaFin}?access_token=${geAccessToken()}`).then(body => body.json()),
}
